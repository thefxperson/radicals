...
for epoch in range(epochs):
    ...
    for batch in range(num_batches):
        ...
        dl_sum = tf.Summary(value=[tf.Summary.Value(tag="Discriminator Loss", simple_value=dl), ])       #tensorboard
        ...
        gl_sum = tf.Summary(value=[tf.Summary.Value(tag="Generator Loss", simple_value=gl), ])       #tensorboard

        #update losses
        writer.add_summary(dl_sum)
        writer.add_summary(gl_sum)
